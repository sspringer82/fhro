...
export class RecipeService {
    private recipes: Recipe[] = [
        ...
        },
        {
            id: 2,
            name: 'Lasagne',
            description: 'Lasagne is a dish of Italian origin, consisting of a layer of thin unleavened bread dough, sometimes called a lasagna sheet, rolled into a long, cylindrical shape, then cooked in a variety of ways.',
            ingredients: [
                { ingredient: { id: 1, name: 'Tomato', unit: 'g' }, amount: 100 },
                { ingredient: { id: 2, name: 'Cheese', unit: 'g' }, amount: 200 },
    ...
    async getFilteredRecipes(ingredientsAvailable: FilterBody): Promise<Recipe[]> {
        const filteredRecipes = this.recipes.filter(recipe => {
            return recipe.ingredients.every(ingredient => {
                const matched = ingredientsAvailable.find(filterIngredient => filterIngredient.ingredient === ingredient.ingredient.name);
